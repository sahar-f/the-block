...
export function ImageGallery({ vehicle }: ImageGalleryProps) {
	...
	const placeholderSrc = getPlaceholderUrl(vehicle);
				...
				<div className="mt-3 flex gap-2 overflow-x-auto pb-1 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
					{indices.map((i) => (
						<button
							...
							className={`relative aspect-video w-20 shrink-0 overflow-hidden rounded-md transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-amber-500 ${
								i === activeIndex
									? "ring-2 ring-accent"
